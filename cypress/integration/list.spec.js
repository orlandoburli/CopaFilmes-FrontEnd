...
context('Movies List', () => {
  context('Server online', () => {
    ...
    it('should present list the 16 movies', () => {
      cy.wait(200)
      cy.get('body').find('.movie-item').then(list => {
        expect(list).to.have.length(16)
      });
    })

    it('should present movie with id tt5164214', () => {
      cy.wait(200)
      const movie = { "id": "tt5164214", "titulo": "Oito Mulheres e um Segredo", "ano": 2018, "nota": 6.3 };

      cy.get(`#movie-${movie.id}`)
        .should('be.visible')
        .find('.movie')
        .should('contain', movie.titulo)
        .should('contain', movie.ano)
    })
  })
  ...
  context('Server offline', () => {
    ...
    it('should present error message', () => {
      ...
      cy.get('body')
        .find('.alert')
        .should('contain', 'Ocorreu um erro ao buscar os dados de filmes')
    })
  })
})
